<x-app-layout>
    <x-slot name="header">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <form action="/categorie/edit/{{ $categories->id }}" method="post">
                {{ csrf_field() }}
                @method('put')
                <div class="mb-6">
                    ...
                    {{$message}}
                    @enderror
                    <button type="submit" value="{{$categories->name}}" class="text-gray-800 bg-amber-300 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-amber-300 dark:hover:bg-amber-100 dark:focus:ring-gray-800">Submit</button>
            </form>
        </div>
    </div>
</x-app-layout>